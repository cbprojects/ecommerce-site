...
export class Util {
  ...
  constructor(public textProperties: TextProperties, public omi: ObjectModelInitializer, public enumerados: Enumerados, public sesionService: SesionService, dataFunctions: Functions) {
    ...
    this.modeloTablas = this.omi.getDataTable();
  }
  ...
  showErrorMessage(error: any, summary: string, severity: string, sticky: boolean = false) {
    console.log(error);
    let msg = { severity: severity, summary: summary, detail: error.error !== null ? `${error.error.mensaje} ${error.error.detalles}` : error.message, moreDetail: error.error !== null ? error.error.detail : "", sticky: sticky };
